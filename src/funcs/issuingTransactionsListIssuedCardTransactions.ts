 ...
 */
export async function issuingTransactionsListIssuedCardTransactions(
  client: MoovCore,
  ...
  Result<
    Array<components.IssuedCardTransaction>,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListIssuedCardTransactionsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/issuing/{accountID}/card-transactions")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, z.array(components.IssuedCardTransaction$inboundSchema)),
