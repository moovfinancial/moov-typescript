 ...
 */
export function disputesUploadEvidenceText(
  client: MoovCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UploadDisputeEvidenceTextRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.CreateEvidenceText, {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.UploadDisputeEvidenceTextResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
