 ...
 */
export async function issuingTransactionsListIssuedCardAuthorizations(
  client: MoovCore,
  ...
  Result<
    Array<components.IssuedCardAuthorization>,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListIssuedCardAuthorizationsRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/issuing/{accountID}/authorizations")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, z.array(components.IssuedCardAuthorization$inboundSchema)),
