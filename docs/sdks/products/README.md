...
<!-- UsageSnippet language="typescript" operationID="listProductCategories" method="get" path="/product-categories" -->
```typescript
import { Moov } from "@moovio/sdk";
...
```typescript
import { MoovCore } from "@moovio/sdk/core.js";
import { productsListCategories } from "@moovio/sdk/funcs/productsListCategories.js";
...
async function run() {
  const res = await productsListCategories(moov);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
